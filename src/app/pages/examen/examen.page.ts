...
import { IonContent, IonHeader, IonToolbar, IonTitle, IonButton, IonCard, IonCardHeader, IonCardTitle, IonCardContent, IonList, IonItem, IonLabel, IonProgressBar, IonRadioGroup, IonRadio, IonButtons, IonMenuButton, IonSpinner, IonCardSubtitle } from '@ionic/angular/standalone';
...
interface Pregunta {
  ...
  correcta: number;
  explicacion: string;
  treeId?: string; // Para identificar a qué árbol pertenece
}
...
@Component({
  ...
  imports: [IonCardSubtitle, 
    CommonModule,
    IonContent, IonHeader, IonToolbar, IonTitle, IonCard, IonCardHeader, IonCardTitle, IonCardContent,
...
export class ExamenPage implements OnInit {
  ...
  cargarPreguntas() {
    // Tomar solo 35 preguntas aleatorias
    const copia = [...preguntasJson];
    copia.sort(() => Math.random() - 0.5);
    const seleccionadas = copia.slice(0, 35);

    // Mapear a la interfaz Pregunta
    this.preguntas = seleccionadas.map((p, index) => ({
      id: index + 1,
      texto: p.question,
      opciones: p.options,
